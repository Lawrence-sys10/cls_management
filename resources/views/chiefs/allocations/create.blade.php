...
@section('subtitle', 'Allocate land to a client')
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-lg-10">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('chief.allocations.store') }}" method="POST" id="allocationForm">
                        @csrf
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control select2-client" 
                                            ...
                                            data-ajax-url="{{ route('chief.clients.search') }}"
                                            required>
                                        @if($selectedClient ?? false)
                                            <option value="{{ $selectedClient->id }}" selected>
                                                {{ $selectedClient->full_name }} - {{ $selectedClient->id_number }} - {{ $selectedClient->phone }}
                                            </option>
                                        @endif
                                    </select>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control select2-land" 
                                            ...
                                            data-ajax-url="{{ route('chief.lands.search') }}"
                                            required>
                                        @if($selectedLand ?? false)
                                            <option value="{{ $selectedLand->id }}" selected>
                                                Plot {{ $selectedLand->plot_number }} - {{ $selectedLand->location }} - {{ $selectedLand->size }} acres
                                            </option>
                                        @endif
                                    </select>
                                    <small class="form-text text-muted">
                                        ...
                                        @if($lands->count() > 0)
                                            @foreach($lands as $land)
                                                <div class="col-md-6 mb-2">
                                                    <div class="card land-card {{ ($selectedLand ?? false) && $selectedLand->id == $land->id ? 'border-primary selected' : '' }}" 
                                                         data-land-id="{{ $land->id }}"
                                                         style="cursor: pointer;">
                                                        <div class="card-body py-2">
                                                            <h6 class="card-title mb-1">Plot {{ $land->plot_number }}</h6>
                                                            <p class="card-text text-muted mb-1 small">{{ Str::limit($land->location, 40) }}</p>
                                                            <div class="d-flex justify-content-between align-items-center">
                                                                <small class="text-muted">{{ $land->size }} acres</small>
                                        ...
                                        @else
                                            <div class="col-12">
                                                <div class="alert alert-warning">
                                                    ...
                                                    <a href="{{ route('chief.lands.create') }}" class="alert-link">Create new lands</a>.
                                                </div>
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('duration_years') is-invalid @enderror" 
                                                    id="duration_years" 
                                                    name="duration_years"
                                                    required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('terms') is-invalid @enderror" 
                                              ...
                                              name="terms" 
                                              rows="3" 
                                              placeholder="Any additional terms and conditions for this allocation...">{{ old('terms') }}</textarea>
                                    @error('terms')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <!-- Form Actions -->
                        <div class="d-flex justify-content-between align-items-center">
                            <a href="{{ route('chief.allocations.index') }}" class="btn btn-secondary">
...
<script>
    $(document).ready(function() {
        ...
        @if($selectedLand ?? false)
            const selectedLand = {
                plot_number: '{{ $selectedLand->plot_number }}',
                location: '{{ $selectedLand->location }}',
                size: '{{ $selectedLand->size }}',
                ownership_status: '{{ $selectedLand->ownership_status ?? "Available" }}'
            };
