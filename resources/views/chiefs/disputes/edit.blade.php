...
@section('title', 'Edit Dispute - ' . $dispute->case_number)
@section('subtitle', 'Update dispute information')
        ...
        <div class="col-md-10">
            <div class="card shadow-sm">
                <div class="card-header bg-white">
                    <h5 class="mb-0">Edit Dispute - {{ $dispute->case_number }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('chief.disputes.update', $dispute) }}" method="POST">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="card-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control @error('case_number') is-invalid @enderror" 
                                                   id="case_number" name="case_number" value="{{ old('case_number', $dispute->case_number) }}" 
                                                   required>
                                            @error('case_number')
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('dispute_type') is-invalid @enderror" 
                                                ...
                                                <option value="boundary" {{ old('dispute_type', $dispute->dispute_type) == 'boundary' ? 'selected' : '' }}>Boundary Dispute</option>
                                                <option value="ownership" {{ old('dispute_type', $dispute->dispute_type) == 'ownership' ? 'selected' : '' }}>Ownership Dispute</option>
                                                <option value="inheritance" {{ old('dispute_type', $dispute->dispute_type) == 'inheritance' ? 'selected' : '' }}>Inheritance Dispute</option>
                                                <option value="other" {{ old('dispute_type', $dispute->dispute_type) == 'other' ? 'selected' : '' }}>Other</option>
                                            </select>
                                            @error('dispute_type')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="date" class="form-control @error('filing_date') is-invalid @enderror" 
                                                   id="filing_date" name="filing_date" value="{{ old('filing_date', $dispute->filing_date->format('Y-m-d')) }}" 
                                                   required>
                                            @error('filing_date')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('severity') is-invalid @enderror" 
                                                ...
                                                <option value="low" {{ old('severity', $dispute->severity) == 'low' ? 'selected' : '' }}>Low</option>
                                                <option value="medium" {{ old('severity', $dispute->severity) == 'medium' ? 'selected' : '' }}>Medium</option>
                                                <option value="high" {{ old('severity', $dispute->severity) == 'high' ? 'selected' : '' }}>High</option>
                                                <option value="critical" {{ old('severity', $dispute->severity) == 'critical' ? 'selected' : '' }}>Critical</option>
                                            </select>
                                            @error('severity')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('status') is-invalid @enderror" 
                                                ...
                                                <option value="pending" {{ old('status', $dispute->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                                <option value="investigation" {{ old('status', $dispute->status) == 'investigation' ? 'selected' : '' }}>Investigation</option>
                                                <option value="hearing" {{ old('status', $dispute->status) == 'hearing' ? 'selected' : '' }}>Hearing</option>
                                                <option value="resolved" {{ old('status', $dispute->status) == 'resolved' ? 'selected' : '' }}>Resolved</option>
                                                <option value="closed" {{ old('status', $dispute->status) == 'closed' ? 'selected' : '' }}>Closed</option>
                                            </select>
                                            @error('status')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                    ...
                                    <div class="card-body">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('land_id') is-invalid @enderror" 
                                                ...
                                                @foreach($lands as $land)
                                                    <option value="{{ $land->id }}" {{ old('land_id', $dispute->land_id) == $land->id ? 'selected' : '' }}>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('complainant_id') is-invalid @enderror" 
                                                ...
                                                @foreach($clients as $client)
                                                    <option value="{{ $client->id }}" {{ old('complainant_id', $dispute->complainant_id) == $client->id ? 'selected' : '' }}>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('respondent_id') is-invalid @enderror" 
                                                ...
                                                @foreach($clients as $client)
                                                    <option value="{{ $client->id }}" {{ old('respondent_id', $dispute->respondent_id) == $client->id ? 'selected' : '' }}>
                                                        {{ $client->full_name }} - {{ $client->id_number }}
                                                    </option>
                                                @endforeach
                                            </select>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                              ...
                                              required>{{ old('description', $dispute->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <a href="{{ route('chief.disputes.show', $dispute) }}" class="btn btn-outline-secondary me-2">
                                    <i class="fas fa-eye me-2"></i>View
                                </a>
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save me-2"></i>Update Dispute
                                </button>
                            </div>
                        </div>
                    </form>
